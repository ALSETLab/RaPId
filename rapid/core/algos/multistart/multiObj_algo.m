function [ sol, other] = multiObj_algo(rapidSettings)
...
%%
options = eval(rapidSettings.multiObjSettings);
funcwrapper = @(x) parfunc(x, rapidSettings);
if isempty(options)
    sol = paretosearch(funcwrapper,length(rapidSettings.experimentSettings.p_0),[],[],[],[],rapidSettings.experimentSettings.p_min,rapidSettings.experimentSettings.p_max);
else
    sol = paretosearch(funcwrapper,length(rapidSettings.experimentSettings.p_0),[],[],[],[],rapidSettings.experimentSettings.p_min,rapidSettings.experimentSettings.p_max,[]);
end
other = [];
