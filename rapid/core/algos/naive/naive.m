function [sol,other]  = naive(rapidSettings,func)
...
%% NAIVE_ALGO
other = [];
part = particle(rapidSettings.experimentSettings.p_0,rapidSettings.experimentSettings.p_0,rapidSettings.experimentSettings.p_0);
part.p = rapidSettings.experimentSettings.p_0;
part.fitness = func(part.p,rapidSettings);
verbose=rapidSettings.experimentSettings.verbose;
% while abs(initialFitness - particle.fitness) > settings.tolerance1
for k=1:rapidSettings.naiveSettings.iterations3
    noSolutionYet = true;
    while noSolutionYet
    ...
    for l=1:rapidSettings.naiveSettings.iterations2
        if verbose
        ...
        for i = 1:length(part.p)
            ...
            for j=1:rapidSettings.naiveSettings.iterations
                if verbose
                sprintf(strcat('inner ',int2str(j)))
                end
                alpha = 1;
                fitnessPos = part.fitness;
                while alpha > rapidSettings.naiveSettings.tolerance2
                    particle2 = part;
                    particle2.p = alpha*P(:,i)'+part.p;
                    particle2.fitness = func(particle2.p,rapidSettings);
                ...
                alpha = -1;
                while abs(alpha) > rapidSettings.naiveSettings.tolerance2
                    particle2 = part;
                    particle2.p = alpha*P(:,i)'+part.p;
                    particle2.fitness = func(particle2.p,rapidSettings);
                    if part.fitness > particle2.fitness
